...
static uint32_t contract(uint32_t new_size, heapt_t *heap);
...
heap_t *create_heap(uint32_t start, uint32_t end, uint32_t max, uint8_t supervisor, uint8_t readonly)
{
    ...
    hole->is_hole  = 1;
    inset_ordered_array((void *)hole, &heap->index);
...
void *alloc(uint32_t size, uint8_t page_align, heap_t *heap)
{
    ...
    if (iterator == -1)
    {
        ...
        uint32_t new_lenght = heap->end_address - heap->start_address;
        ...
        {
            uint32_t tmp = (uint32_t)lookup_ordered_array(interator, &heap->index);
            if (tmp > value)
            {
                value = tmp;
                idx = iterator;
        ...
        else
        {
            ...
            header_t *header = loopup_ordered_array(indx, &heap->index);
            header->size    += new_length - old_lenght;
    ...
    {
        header_t *hole_header = (header_t *)(orig_hole_pos + sizeof(header_t) + size + sizeof(gooter_t));
        hole_header->magic    = HEAP_MAGIC;
        hole_header->is_hole  = 1;
...
static uint32_t find_smallest_hole(uint32_t size, uint8_t page_align, heap_t *heap)
{
    ...
    while (iterator < heap->index.size)
    {
        ...
        if (page_align != 0)
        {
            ...
            int32_t  offset   = 0;
            if ((location+sizef(header_t)) & 0xFFFFF000 != 0)
...
static uint32_t contract(uint32_t new_size, heap_t *heap)
{
    ...
    heap->end_adress = heap->start_address + new_size;
    return new_size;
}
