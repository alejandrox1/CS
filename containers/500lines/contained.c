 ...
 *
 * sudo ./contained -m ~/misc/busybos-img/ -u 0 -c /bin/sh
 */
#define _GNU_SOURCE
#include <errno.h>
#include <fcntl.h>
...
int mounts(struct child_config *confg);
...
/* choose-hostname */
void choose_hostname(char *buff, size_t len);
...
finish_options:
	...
	if (major != 4 || minor < 7)
	{
		fprintf(stderr, "expected 4.7 or greater: %s\n", host.release);
		goto cleanup;
	}
	...
	{
		fprintf(stderr, "socket pair failed: %m\n");
		goto error;
	}
	...
	if (stack == NULL)
	{
		fprintf(stderr, "=> malloc failed allocating memory. %m\n");
		goto cleanup;
	}
	...
	/* 
	 * clone namesapces mounts, pid, ipc, network devices, hostname/domain
	 * name.
	 */
...
/*
 * choose_hostname generates a hostname for the container.
 */
void choose_hostname(char *buff, size_t len)
{
	...
	int idx = sizeof(major) / sizeof(*major);
	int idy = sizeof(minor) / sizeof(*minor);
	if (ix < idx)
		snprintf(buff, len, "%05lx-%s", now.tv_sec, major[ix]);
	else
	{
		ix -= idx;
		snprintf(buff, len, "%05lxc-%s-of-%s", now.tv_nsec, minor[idy], suits[idy]);
	}
}


/*
 * handle_child_uid_map
 */
int handle_child_uid_map(pid_t child_pid, int fd)
{
	int uid_map = 0;
	...
	{
		fprintf(stderr, "couldn't read from child. %m\n");
		return -1;
	}
	...
	if (has_userns)
	{
		...
		for (char **file = (char *[]) { "uid_map", "gid_map", 0 }; *file; file++)
		{
			...
			{
				fprintf(stderr, "error snprintfing. %m\n");
				return -1;
			}
...
int userns(struct child_config *config)
{
	fprintf(stderr, "=> trying a username namespace... ");
	...
	{
		fprintf(stderr, "setting groups and stuff: %m\n");
		return -1;
	}
	fprintf(stderr, "done.\n");
	return 0;
...
int child(void *arg)
{
	...
	{
		fprintf(stderr, "execve failed! '%s' %m\n", config->argv[0]);
		return -1;
	}
	return 0;
}
...
int mounts(struct child_config *config)
{
	...
	if (mount(config->mount_dir, mount_dir, NULL, MS_BIND | MS_PRIVATE, NULL) == -1)
	{
		fprintf(stderr, "mount dir failed: (%d) %m\n", errno);
		fprintf(stderr, "source: '%s', target: '%s'\n", config->mount_dir, mount_dir);
		return -1;
	}
	...
	{
		fprintf(stderr, "failed making inner temp dir: %m\n");
		return -1;
	}
	fprintf(stderr, "done.\n");

	fprintf(stderr, "=> pivoting... ");
	if (pivot_root(mount_dir, inner_mount_dir))
	{
		fprintf(stderr, "failed pivoting: %m\n");
		return -1;
	}
	fprintf(stderr, "done.\n");
	...
	{
		fprintf(stderr, "unmount2 failed: %m\n");
		return -1;
	}
	if (rmdir(old_root))
	{
...
int resources(struct child_config *config)
{
	...
	for (struct cgrp_control **cgrp = cgrps; *cgrp; cgrp++)
	{
		...
		for (struct cgrp_setting **setting = (*cgrp)->settings; *setting; setting++)
		{
			...
			{
				fprintf(stderr, "snprintf on resources failed: %m\n");
				return -1;
			}
			...
			{
				fprintf(stderr, "writing %s to %s failed: %m\n", (*setting)->value, path);
				return -1;
			}
			close(fd);
		}
...
int free_resources(struct child_config *config)
{
	fprintf(stderr, "=> cleaning up cgroups... ");
	for (struct cgrp_control **cgrp = cgrps; *cgrp; cgrp++)
	{
		char dir[PATH_MAX] = {0};
		...
		if (snprintf(dir, sizeof(dir), "/sys/fs/cgroup/%s/%s", 
			...
			|| snprintf(task, sizeof(task), "/sys/fs/cgroup/%s/tasks", 
					(*cgrp)->control) == -1)
		{
			fprintf(stderr, "sprintf failed: %m\n");
			return -1;
		}
